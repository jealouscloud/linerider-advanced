...
namespace QuickFont
{
	...
	class Builder
	{
		...
		public static QFontData LoadQFontDataFromFile(List<Bitmap> bmps, byte[] qfont, float downSampleFactor, QFontLoaderConfiguration loaderConfig)
		{
			var lines = new List<String>();
			using (MemoryStream ms = new MemoryStream(qfont))
			{
			...
			for (int i = 0; i < pageCount; i++)
				bitmapPages.Add(new QBitmap(bmps[i]));
