...
namespace linerider.Drawing
{
    internal class TrackRenderer : GameService
    {
        ...
        private void AddLineIndices(Line l)
        {
            ...
            var lv = _lines[l.ID];
            bool hittest = Settings.Local.HitTest && game.Track.PlaybackMode && collisions.Contains(l.ID);
            ...
            using (new GLEnableCap(EnableCap.Texture2D))
            {
                ...
                try
                {
                    ...
                    if (RequiresUpdate || _prepped_knobs != r_knobs || _prepped_knobs_red != r_knobs_red ||
                        ...
                        (Settings.Local.HitTest && game.Track.PlaybackMode && !track.AllCollidedLines.SetEquals(collisions)))
                    {
                        lock (SyncRoot)
                        {
        ...
        private List<int> DrawGWell(StandardLine line)
        {
            ...
            var angle = Angle.FromLine(p1, p2);
                    ...
                    case LineType.Blue:
                        {
                            ...
                            var l = line;
                            var loc3 = (float)(l.DiffNormal.X > 0 ? (Math.Ceiling(l.DiffNormal.X)) : (Math.Floor(l.DiffNormal.X)));
                            var loc4 = (float)(l.DiffNormal.Y > 0 ? (Math.Ceiling(l.DiffNormal.Y)) : (Math.Floor(l.DiffNormal.Y)));
